package com.teamarc.olanit.aspect;
...
public class LoggingAspect {
    ...
    @Before("execution(* com.teamarc.olanit.services.*.*(..))")
    ...
    @AfterReturning(pointcut = "execution(* com.teamarc.olanit.services.*.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        String methodName = joinPoint.getSignature().getName();
        log.info("Exiting method: {}() with result: {}", methodName, result != null ? result : "void");
    }

    @AfterThrowing(pointcut = "execution(* com.teamarc.olanit.services.*.*(..))", throwing = "error")
    public void logAfterThrowing(JoinPoint joinPoint, Throwable error) {
        String methodName = joinPoint.getSignature().getName();
        log.error("Exception in method: {}() with error: {}", methodName, error.getMessage());
    }

    @Around("execution(* com.teamarc.olanit.services.*.*(..))")
